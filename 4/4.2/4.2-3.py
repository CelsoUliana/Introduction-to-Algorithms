...
# Strassen algorithm modified to work with square matrix without n being power of 2
...
# Should result in [[130, 120, 240, 0],
#                   [51, 47, 73, 0],
#                   [35, 33, 45, 0],
#                   [0, 0, 0, 0]]
...
# Bitwise operations to find next power of 2.
def nextPowerof2(n):
    if n == 0:
        return 1
    if n & (n - 1) == 0:
...
def prepStrassen(A, B):
    n = nextPowerof2(len(A))
    # Create new A and B padded with n²(power of 2) zeroes
    newA = [[0 for _ in range(n)] for _ in range(n)]
    newB = [[0 for _ in range(n)] for _ in range(n)]
    ...
    # Could trim the 0 cols and rows with len(A) size but I'm lazy
    return strassen(newA, newB)
